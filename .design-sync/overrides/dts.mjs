// forked from design-sync lib/dts.mjs - workspace sources outside the shim package (components/*.tsx re-exported from .design-sync/pkg/index.ts) are the DS's own API: keep their on*/aria-* props instead of treating them as inherited DOM noise.
...
function isOwnProp(p, pkgDir, styleSystemDirs) {
  ...
  // FORK: a workspace path (no node_modules/ segment) is this repo's own
  // source - the shim package re-exports components/ from outside its dir.
