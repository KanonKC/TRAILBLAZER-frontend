// forked from design-sync lib/story-imports.mjs - skip the async b.resolve round-trip for imports whose IMPORTER already lives in node_modules (lucide-react's ~1500 icon modules per preview made every preview compile take ~12s on Windows); such imports can never resolve to a workspace component, so the policy outcome is unchanged.
  ...
  const policyRedirect = {
    ...
    setup(b) {
      b.onResolve({ filter: /.*/ }, async (a) => {
        ...
        // FORK: third-party internals resolve natively - see header.
        if (a.importer && a.importer.split(String.fromCharCode(92)).join('/').includes('/node_modules/')) return null;
